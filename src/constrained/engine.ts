...
export function compatibility(a:Dataset,b:Dataset,accepted=false){
 ...
 else {if(!accepted)throw new Error('The images lack geographic alignment metadata. A visual comparison requires assuming the same aligned extent.');warnings.push('Provisional visual comparison: the images are assumed to show the same aligned extent. Geographic alignment is unverified; no registration was performed.');}
