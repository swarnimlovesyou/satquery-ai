...
import { supportedTasks, recommendedTask } from '../src/constrained/capabilities';
...
test('only offers supported tasks and chooses useful defaults without guessing spectral bands',()=>{
 const d=data([[1],[2]]),m={...mapping,green:-1,swir:-1};
 assert.deepEqual(supportedTasks(d,m),['statistics','vegetation']);
 assert.equal(recommendedTask(d,m),'vegetation');
 assert.equal(recommendedTask(d,m,true),'change');
 assert.deepEqual(supportedTasks(d,{...m,category:'unknown'},true),['statistics','change']);
 assert.deepEqual(supportedTasks(d,{...m,category:'sar'}),['statistics','sar']);
});
...
test('rejects mismatched CRS/extents and requires explicit unreferenced alignment confirmation',()=>{
 ...
 b.metadata.bounds=null;assert.throws(()=>compatibility(a,b),/assuming/i);assert.ok(compatibility(a,b,true).length);
});
